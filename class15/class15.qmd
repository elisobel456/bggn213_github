...
Pertussis, aka Whooping Cough, is a highly contagious lung infection caused by the *B. Pertussis*.
...
We need to "scrape" this data so we do stuff with it in R. Let's try the **datapasta** package.

In the console, type #install.packages("datapasta")
...
Let's add the date of wP vaccine roll out completion and the switch to the new aP vaccine.
...
We can use the **jsonlite** package and its `read_json()` function.

Install **jsonlite** using `install.packages()`.
...
```{r}
pt_2021 <- filter(igg, dataset=="2021_dataset")
data_2021 <- filter(pt_2021, antigen=="PT")
```
